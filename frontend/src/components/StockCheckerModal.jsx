...
const StockCheckerModal = ({ product, onClose }) => {
  ...
  const fetchRegions = async () => {
    setError('');
    try {
      const response = await fetch(`/api/products/${product.id}/regions`);
      if (!response.ok) {
  ...
  const checkStock = async () => {
    ...
    try {
      let url;
      if (warehouseFilter) {
        url = `/api/products/${product.id}/stock/check?warehouse=${encodeURIComponent(warehouseFilter)}`;
      } else {
        url = `/api/products/${product.id}/stock/region?region=${encodeURIComponent(selectedRegion)}`;
      }
