...
def network_ping(payload: PingRequest):
    """
    ⚠️ VULNERABLE: OS Command Injection via shell execution
    
    This endpoint has been intentionally made vulnerable to demonstrate
    how OS command injection attacks work. The endpoint accepts user input
    directly in a shell command without validation.
    
    Attack Vector:
    - Accepts special shell operators: &&, |, ;, etc.
    - Uses shell=True which enables shell interpretation
    - String interpolation allows arbitrary command execution
    
    Example Payloads:
    - "8.8.8.8 && whoami" → Executes whoami command
    - "8.8.8.8 && cat /etc/passwd" → Reads sensitive files
    - "8.8.8.8 && id" → Gets user ID (currently root)
    - "8.8.8.8 | whoami" → Using pipe operator
    - "8.8.8.8; id" → Using semicolon separator
    
    Impact: CRITICAL - Remote Code Execution with root privileges
    """
    # ❌ NO VALIDATION - User input directly used in shell command
    # ❌ DANGEROUS: shell=True enables command injection
    # ❌ VULNERABLE: String interpolation of user input
    
    try:
        # This line is VULNERABLE to OS Command Injection
        # The user-supplied payload.target is directly embedded in the shell command
        output = subprocess.check_output(
            f"ping -c 4 {payload.target}",  # String interpolation = DANGER
            shell=True,  # CRITICAL: This enables shell interpretation
