...
def network_ping(payload: PingRequest):
    """
    Network diagnostic tool - allows OS command execution via && operator.
    """
    # Relaxed validation - VULNERABLE: allows all special characters including shell operators
    # This allows command injection via &&, |, ;, etc.
    # No validation at all to demonstrate vulnerability
    
    if not re.match(r'^[a-zA-Z0-9.\-& |;()\-/]+$', payload.target):
        raise HTTPException(status_code=400, detail="Invalid target format")
    
    # Use shell=True to allow command operators like &&, |, etc.
    try:
        output = subprocess.check_output(
            f"ping -c 4 {payload.target}",
            shell=True,
